<script lang="ts">
  ...
  // Assign form state
  let assignTag = $state('');
  let assignPlaylistId: number | null = $state(null);
  ...
  // Time slot editor
  let showSlotEditor = $state(false);
  ...
  let masterData = $derived($master);
  let statusData = $derived($status);
  let slots = $derived($timeSlots);
</script>

<h1 class="text-2xl font-bold text-gray-900 dark:text-white">Master Playlist</h1>
<p class="text-sm text-gray-500 dark:text-gray-400 -mt-4">Assign playlists to time slots. The scheduler automatically switches to the matching slot throughout the day.</p>
...
<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
  <div class="flex items-center justify-between mb-4">
    ...
    {#if !showSlotEditor}
      <button type="button" class="px-4 py-2 text-xs font-semibold text-primary-600 dark:text-primary-400 border border-primary-300 dark:border-primary-700 rounded-lg hover:bg-primary-50 dark:hover:bg-primary-900/30 transition-colors" onclick={openSlotEditor}>Configure</button>
    {/if}
  </div>

  {#if showSlotEditor}
    <TimeSlotEditor slots={$timeSlots} saving={savingSlots} onsave={(e) => saveSlots(e)} oncancel={cancelSlotEditor} />
  {:else}
...
<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
  ...
  <div class="flex flex-col sm:flex-row gap-3">
    <select bind:value={assignTag} class="px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
      {#each slots as slot}
        <option value={slot.tag}>{getTagEmoji(slot.tag)} {slot.label} ({formatHourRange(slot.startHour, slot.endHour)})</option>
      {/each}
    </select>
    <select bind:value={assignPlaylistId} class="flex-1 px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
    ...
    <button type="button" class="px-6 py-2.5 text-sm font-semibold text-white bg-primary-600 hover:bg-primary-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed" onclick={handleAssignToTag} disabled={!assignPlaylistId}>Assign</button>
  </div>
</div>
...
<div class="grid gap-4">
  {#each slots as slot (slot.tag)}
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden {isActive ? 'ring-2 ring-primary-400 dark:ring-primary-600' : ''}">
      <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400">{tagData.count} playlist{tagData.count !== 1 ? 's' : ''} assigned</p>
          </div>
        </div>
      </div>
          ...
          {#each tagData.playlists as pl (pl.id)}
            <div class="px-5 py-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400">{(pl.tracks || []).length} track{(pl.tracks || []).length !== 1 ? 's' : ''} · ID: {pl.id}</p>
              </div>
              <button type="button" class="ml-3 p-2 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 dark:hover:text-red-400 transition-colors" title="Remove from this time slot" onclick={() => handleRemoveFromTag(tag, pl.id)}>
      ...
      {:else}
        <div class="px-5 py-6 text-center text-sm text-gray-400 dark:text-gray-500">No playlists assigned. Use the form above to add one.</div>
      {/if}
    </div>
  {/each}
</div>
...
<div class="flex justify-center">
  <button type="button" class="px-4 py-2 text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors flex items-center gap-2" onclick={() => { master.refresh(); status.refresh(); }}>Refresh</button>
</div>
