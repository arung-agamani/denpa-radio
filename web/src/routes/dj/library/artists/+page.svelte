<script lang="ts">
  ...
  let entries = $derived($tracksByArtist);
...
{:else}
  ...
  {#if filtered.length === 0}
    <div class="flex flex-col items-center justify-center py-16 text-gray-400 dark:text-gray-500">
      ...
      <p class="text-sm font-medium">No artists found</p>
    </div>
  {:else}
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {#each filtered as entry (entry.key)}
        <button
          type="button"
          class="flex items-center gap-4 p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:shadow-md hover:border-primary-300 dark:hover:border-primary-700 transition-all duration-200 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
          onclick={() => goto(`/dj/library/artists/${encodeURIComponent(entry.key)}`)}
        >
          <CoverArt src={entry.coverUrl || ''} alt={entry.key} size="md" />
          <div class="flex-1 min-w-0">
            <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">{entry.key}</p>
