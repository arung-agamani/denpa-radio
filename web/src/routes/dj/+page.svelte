<script lang="ts">
  ...
  import { status, playlists, master, scheduler, timezone, timeSlots, toasts } from '$lib/stores';
  ...
  const commonTimezones = [
    ...
    'Asia/Tokyo', 'Asia/Shanghai', 'Asia/Hong_Kong', 'Asia/Singapore', 'Asia/Jakarta', 'Asia/Kolkata', 'Asia/Dubai', 'Asia/Seoul',
    'Australia/Sydney', 'Australia/Melbourne', 'Australia/Perth',
    'Pacific/Auckland',
    'America/New_York', 'America/Chicago', 'America/Denver', 'America/Los_Angeles', 'America/Sao_Paulo', 'America/Argentina/Buenos_Aires', 'America/Mexico_City',
  ...
  async function handleSetTimezone() {
    savingTimezone = true;
    try {
      const value = timezoneInput.trim() === 'UTC' ? '' : timezoneInput.trim();
  ...
  let schedulerData = $derived($scheduler);
...
<div class="grid grid-cols-2 md:grid-cols-5 gap-4">
  <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
    <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">📚 Library Tracks</p>
    <p class="text-2xl font-bold text-gray-900 dark:text-white">{statusData.library_tracks || 0}</p>
  </div>
  <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
    <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">Playlist Tracks</p>
    <p class="text-2xl font-bold text-gray-900 dark:text-white">{statusData.total_tracks || 0}</p>
  </div>
  <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
    <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">Playlists</p>
    <p class="text-2xl font-bold text-gray-900 dark:text-white">{allPlaylistsList.length}</p>
  </div>
  <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
    <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">Listeners</p>
    <p class="text-2xl font-bold text-gray-900 dark:text-white">{statusData.active_clients || 0}</p>
  </div>
  <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
    <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">Active Tag</p>
...
<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
  <div class="flex items-center justify-between mb-4">
    ...
    <button type="button" class="text-xs text-primary-600 dark:text-primary-400 hover:underline" onclick={() => scheduler.refresh()}>
      Refresh
    </button>
  </div>
...
<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
  <div class="flex items-center justify-between mb-4">
    ...
    <span class="text-xs text-gray-400 dark:text-gray-500">
      Server time: {statusData.server_time
        ? new Date(statusData.server_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })
        : '—'}
    </span>
  </div>
  <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">Set the timezone used for time-based playlist scheduling.</p>
  ...
  <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">Currently: <span class="font-semibold text-gray-600 dark:text-gray-300">{statusData.timezone || 'UTC'}</span></p>
</div>
