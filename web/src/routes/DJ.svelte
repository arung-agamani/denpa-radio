<script>
    ...
    onMount(() => {
        ...
        // Sync timezone input with the current value from the server.
        timezoneInput = $timezone || "UTC";

        // Keep timezone input in sync when status refreshes.
    ...
    // Reactively update timezoneInput when store changes (outside of saving)
    ...
    $: filteredLibraryTracks = trackSearchQuery.trim()
        ? allTracks.filter(
              (t) =>
                  (t.title || "")
                      .toLowerCase()
                      .includes(trackSearchQuery.toLowerCase()) ||
                  (t.artist || "")
                      .toLowerCase()
                      .includes(trackSearchQuery.toLowerCase()) ||
                  (t.album || "")
    ...
    <div class="flex-1 overflow-y-auto bg-gray-50 dark:bg-gray-900">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
            ...
            {:else if $djActiveSection === "tracks"}
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 -mt-4">
                    Central library of all known tracks. Edit metadata here and
                    changes propagate to all playlists.
                </p>
                ...
                >
                    <div
                        class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3"
                    >
                        <h3
                            class="text-base font-semibold text-gray-900 dark:text-white"
                        >
                    ...
                    {#if allTracks.length === 0 && !loadingTracks}
                        <div
                            class="p-12 text-center text-gray-400 dark:text-gray-500"
                        >
                            <span class="text-4xl block mb-3">📚</span>
                            <p class="text-sm font-medium">
                                Track library is empty.
                            </p>
                            <p class="text-xs mt-1">
                                Click "Load Library" to fetch all tracks, or
                                "Scan Music Directory" to discover files.
                            </p>
                        </div>
                    ...
                    {:else}
                        <div class="overflow-x-auto">
                            <table class="w-full text-left">
                                <thead>
                                    <tr
                                        class="border-b border-gray-200 dark:border-gray-700 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400"
                                    >
                                        <th class="px-3 py-2 w-16 text-center"
                                            >ID</th
                                        >
                                        ...
                                        >
                                        <th
                                            class="px-3 py-2 hidden md:table-cell"
                                            >Album</th
                                        >
                                        <th
                                            class="px-3 py-2 hidden lg:table-cell w-20 text-center"
                                            >Format</th
                                        >
                                        <th class="px-3 py-2 w-32 text-center"
                                            >Actions</th
                                        >
                                    </tr>
                                </thead>
                                <tbody>
                                    {#each filteredLibraryTracks.slice(0, 200) as track (track.id || track.checksum)}
                                        {#if editingTrackId === track.id}
                                            <!-- Inline edit row -->
                                            <tr
                                                class="border-b border-gray-100 dark:border-gray-800 bg-primary-50 dark:bg-primary-900/20"
                                            >
                                                <td
                                        ...
                                        {:else}
                                            <!-- Normal display row -->
                                            <tr
                                                class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 group"
                                            >
                                                <td
                                                    class="px-3 py-2 text-center"
                                                >
                                                ...
                                                <td class="px-3 py-2">
                                                    ...
                                                    {#if track.artist}
                                                        <p
                                                            class="text-xs text-gray-500 dark:text-gray-400 truncate sm:hidden mt-0.5"
                                                        >
                                                            {track.artist}
                                                        </p>
                                                    {/if}
                                                ...
                                                >
                                                    <span
                                                        class="text-sm text-gray-600 dark:text-gray-300 truncate block max-w-[200px]"
                                                        >{track.artist ||
                                                            "—"}</span
                                                    >
                                                </td>
                                                ...
                                                >
                                                    <span
                                                        class="text-sm text-gray-500 dark:text-gray-400 truncate block max-w-[200px]"
                                                        >{track.album ||
                                                            "—"}</span
                                                    >
                                                </td>
                                                <td
                                                    class="px-3 py-2 hidden lg:table-cell text-center"
                                                >
                                                    {#if track.format}
                                                        <span
                                                ...
                                                >
                                                    <div
                                                        class="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity"
                                                    >
                                                        <!-- Edit -->
                                                        <button
                                                            type="button"
                        ...
                        {#if filteredLibraryTracks.length > 200}
                            <div
                                class="px-4 py-3 text-center text-xs text-gray-400 dark:text-gray-500 border-t border-gray-100 dark:border-gray-800"
                            >
                                Showing 200 of {filteredLibraryTracks.length} tracks.
                                Use search to narrow results.
                            </div>
                        {/if}
                        <div
                            class="px-3 py-2 text-xs text-gray-400 dark:text-gray-500 border-t border-gray-100 dark:border-gray-800"
                        >
                            {filteredLibraryTracks.length} track{filteredLibraryTracks.length !==
                            1
                                ? "s"
                                : ""} in library
                        </div>
                        ...
                        >
                            <div class="text-center">
                                ...
                                <p
                                    class="text-sm text-gray-500 dark:text-gray-400 mb-6"
                                >
                                    This will remove the track from the library <strong
                                        >and all playlists</strong
                                    > that contain it. The file on disk will not be
                                    deleted.
                                </p>
                                <div class="flex gap-3 justify-center">
                                    <button
