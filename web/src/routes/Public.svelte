<script>
  ...
  async function loadTracks() {
    ...
    try {
      ...
      error = err.message || 'Failed to load track list';
    } finally {
      loading = false;
    }
  }
  ...
  $: displayTracks = showFullPlaylist ? tracks : tracks.slice(0, 25);
  $: hasMore = tracks.length > 25 && !showFullPlaylist;
</script>
  ...
  <section>
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-3">
        <h2 class="text-lg font-bold text-gray-900 dark:text-white">Playlist</h2>
        {#if !loading}
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      {:else if error}
        <div class="flex flex-col items-center justify-center py-16 px-4">
          ...
          <p class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Could not load playlist</p>
      ...
      {:else}
        <TrackList
          ...
          emptyMessage="No tracks available yet. Add some music to the music directory!"
        />
        ...
        {#if hasMore}
          <div class="px-4 py-3 border-t border-gray-100 dark:border-gray-800 text-center">
            <button
              ...
              on:click={() => (showFullPlaylist = true)}
            >
        ...
        {:else if showFullPlaylist && tracks.length > 25}
          <div class="px-4 py-3 border-t border-gray-100 dark:border-gray-800 text-center">
            <button
              type="button"
              class="text-sm font-medium text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 transition-colors"
              on:click={() => (showFullPlaylist = false)}
            >
              Show fewer tracks
              <svg class="inline-block w-4 h-4 ml-1" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
